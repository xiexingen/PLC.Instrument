...
namespace PLC.Instrument.Tests
{
    public abstract class InstrumentTestBase : AbpIntegratedTestBase<InstrumentTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected InstrumentTestBase()
        {
        ...
        protected void UsingDbContext(Action<InstrumentDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<InstrumentDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<InstrumentDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<InstrumentDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<InstrumentDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<InstrumentDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<InstrumentDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<InstrumentDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<InstrumentDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<InstrumentDbContext, Task<T>> func)
        {
            T result;
